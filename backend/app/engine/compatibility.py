...
def compute_compatibility_score(
    ...
    farmer_var = farmer_produce.variety.strip().lower()
    team_var = team.variety.strip().lower()
    if farmer_var == team_var or farmer_var in ["all", "standard", "any"] or team_var in ["all", "standard", "any"]:
        variety_score = 1.0
    ...
    farmer_grade = farmer_produce.grade.strip().upper()
    team_grade = team.grade.strip().upper()
    ...
    projected_total_kg = team_total_kg + farmer_produce.available_quantity_kg
    ...
    if unlocked_target:
        quantity_score = 1.0
        quantity_reason = f"Crucial quantity contribution (+{farmer_produce.available_quantity_kg:g} kg unlocks bulk buyer tier)"
    elif projected_total_kg >= 1000:
        quantity_score = 0.90
        quantity_reason = f"Brings combined lot to {projected_total_kg:g} kg for commercial buyers"
    elif farmer_produce.available_quantity_kg >= 200:
        quantity_score = 0.80
        quantity_reason = f"Healthy produce contribution of {farmer_produce.available_quantity_kg:g} kg"
    else:
        quantity_score = 0.60
        quantity_reason = f"Smaller lot contribution of {farmer_produce.available_quantity_kg:g} kg"
